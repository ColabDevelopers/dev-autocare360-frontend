...
export function useMessaging(userId: number): UseMessagingReturn {
  ...
  const subscribe = useCallback(() => {
    ...
    if (!subscriptionRef.current) {
      subscriptionRef.current = client.subscribe(`/user/queue/messages`, (message: IMessage) => {
        const newMessage = JSON.parse(message.body) as Message
      ...
      console.log(
        `✅ Subscribed to /user/queue/messages (will receive messages for current authenticated user)`
      )
    }
